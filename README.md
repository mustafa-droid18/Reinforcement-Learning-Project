...
All code in this repository was written from scratch for this project. No external starter repo was used.

| Component | Description |
|-----------|-------------|
| `src/mario_rl/` | Training scaffold written for this project: PPO training loop, vectorised environment builder, custom Gym wrappers (frame skip, stagnation termination, reward shaping), config dataclasses, eval and video utilities |
| `llm_reward_loop.py` | Automated LLM reward generation loop — calls Claude API, trains PPO, feeds back training diagnostics, iterates over 5 rounds |
| `reward_functions/human_heuristic.py` | Hand-designed reward function, developed and iterated manually over 8 tuning rounds |
| `reward_functions/llm/` | Reward functions generated by Claude via the automated loop (llm_v1_r1–r5), plus the two selected for 5M final runs |
| `configs/` | All experiment JSON configs |

**Third-party libraries used (not modified):** `stable-baselines3`, `gym-super-mario-bros`, `nes-py`, `torch`, `anthropic`.
