...
TRAIN_CONFIG = "configs/llm/llm_v1_test_r1.json"
PYTHONPATH = "src"

REWARD_DIR = Path("reward_functions/llm")
...
def run_training(round_num: int, reward_path: Path) -> bool:
    ...
    tmp_config = Path(f"configs/llm/llm_v1_test_r{round_num}.json")
...
def eval_best_model(round_num: int, n_episodes: int = 20) -> dict | None:
    ...
    try:
        ...
        config_path = Path(f"configs/llm/llm_v1_test_r{round_num}.json")
...
def main() -> None:
    ...
    print(f"Run the final 5M training with: PYTHONPATH=src python -m mario_rl.train --config configs/llm/llm_v1_final.json")
